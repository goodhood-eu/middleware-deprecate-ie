(function() {
  ...
  const getSelector = (selector) => btoa(`middleware-deprecate-ie-${selector}`);
  ...
  inject('head', `
    <style type="text/css">
      ...
      .${getSelector('modal')} a:visited {
        color: #799189;
      }
      .${getSelector('modal')} a:active,
      .${getSelector('modal')} a:hover {
        color: #575757;
      }
  ...
  inject('body', `
    <div class={${getSelector('modal')}}>
      ...
      <div class="${getSelector('controls')}">
        <span id="${'js-cookie-pass'}" class="${getSelector('button')}">${lang.link}</span>
      </div>
    </div>
  `);
}(document));
